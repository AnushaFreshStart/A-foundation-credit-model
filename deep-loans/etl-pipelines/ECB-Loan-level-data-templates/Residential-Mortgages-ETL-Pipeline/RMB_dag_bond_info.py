...
PYTHON_FILE_LOCATION = f"gs://{CODE_BUCKET}/dist/rmb_main.py"
...
PY_FILES = f"gs://{CODE_BUCKET}/dist/rmb_etl_pipeline_0.1.0.zip"
...
RUNTIME_CONFIG = {
    "properties": {
        "spark.app.name": "rmb_etl_pipeline",
        ...
        set(
            [
                ...
                for b in storage_client.list_blobs(
                    bucket.name,
                    prefix="esmasecreg_data/downloaded-data/RMB",
                )
...
# PROFILE + BRONZE
# with models.DAG(
#     "rmb_bond_info",  # The id you will see in the DAG airflow page
...
#         # bond_info TaskGroup
#         with TaskGroup(group_id=f"{dl_code}_bond_info") as tg:
...
#                             f"--source-prefix=esmasecreg_data/downloaded-data/RMB/{dl_code}",
#                             "--file-key=Bond_Info",
...
#                 },
#                 batch_id=f"{dl_code.lower()}-bond-info-profile",
#             )
...
#                             f"--data-bucketname={DATA_BUCKET}",
#                             f"--source-prefix=esmasecreg_data/downloaded-data/RMB/{dl_code}",
#                             "--target-prefix=RMB/bronze/bond_info",
...
#                     "runtime_config": RUNTIME_CONFIG,
#                 },
#                 batch_id=f"{dl_code.lower()}-bond-info-bronze",
#             )
...
with models.DAG(
    ...
    for rp in raw_prefixes:
        ...
        silver_task = DataprocCreateBatchOperator(
            ...
            batch={
                "pyspark_batch": {
                    ...
                    "args": [
                        ...
                        f"--raw-bucketname={RAW_BUCKET}",
                        f"--data-bucketname={DATA_BUCKET}",
                        "--source-prefix=RMB/bronze/bond_info",
                        "--target-prefix=RMB/silver/bond_info",
                        f"--deeploans-code={dl_code}",
            ...
            batch_id=f"{dl_code.lower()}-bond-info-silver",
        )
        # clean-up TaskGroup
